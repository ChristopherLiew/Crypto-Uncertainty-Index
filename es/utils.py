...
# Data Processing
def es_reddit_to_df(
    results: Union[Response, List[Dict[str, Any]]],
    input_type: str = "dsl",
    output_type: str = "pandas",
) -> pd.DataFrame:

    assert input_type in ("dsl", "es"), ValueError(
        'Input type must be one of "dsl" or "es"'
    )
        ...
        results_list = [
            {
                ...
                "create_datetime": datetime.strptime(
                    hit["_source"]["create_datetime"], DATETIME_FORMAT
                ),
        ...
        results_list = [
            {
                "id": hit.id,
                "create_datetime": datetime.strptime(
                    hit.create_datetime, DATETIME_FORMAT
                ),
